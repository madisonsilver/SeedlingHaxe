...
load_image_assets();
	...
	override public function render():Void // (FP.world as Game).drawCover(0, 0.75);
	{
		drawCover(alpha);
	...
	private function renderSeal(j:Int):Void {
		sprSeal.render(new Point(FP.screen.width / 2
			+ scale * sprSeal.originX
			+ scale * sprSeal.width * (Math.floor(j / rows) - cols / 2),
			FP.screen.height / 2
			+ scale * sprSeal.originY
			+ scale * sprSeal.height * (j % rows - rows / 2)),
			new Point());
	}
