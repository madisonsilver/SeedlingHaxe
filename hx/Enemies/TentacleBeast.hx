    ...
    override public function update() : Void
    {
        ...
        if (activated && !dead)
        {
            if (sprTentacleBeast.currentAnim == "")
            {
                ...
                {
                    xpos = as3hx.Compat.parseInt(Math.random() * (spawnRect.width - whirlpoolMargin.x - whirlpoolMargin.width) + spawnRect.x + whirlpoolMargin.x);
                    ypos = as3hx.Compat.parseInt(Math.random() * (spawnRect.height - whirlpoolMargin.y - whirlpoolMargin.height) + spawnRect.y + whirlpoolMargin.y);
                ...
                if (FP.world.classCount(Tentacle) < maxTentacles)
                {
                    ...
                    ypos = as3hx.Compat.parseInt(Math.random() * (spawnRect.height - tentacleMargin.y - tentacleMargin.height) + spawnRect.y + tentacleMargin.y);
