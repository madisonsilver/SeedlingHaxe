...
private var imgShield : Class<Dynamic>;
    ...
    private var moveSpeeds(default, never) : Array<Dynamic> = [dMS, dMSwater, dMS, dMS, dMS, dMS, dMS, dMS, dMS, dMS, 
    ...
    private var walkAnimSpeeds(default, never) : Array<Dynamic> = [dwAS, dwASwater, dwAS, dwAS, dwAS, dwAS, dwAS, dwAS, dwAS, dwAS, 
    ...
    private var standAnimSpeeds(default, never) : Array<Dynamic> = [dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, 
        dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, 
        dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, 
        dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS, dsAS];
    
    private var hitables : Dynamic = ["Enemy", "Grass", "Tree", "Rock", "Rope", "ShieldBoss", "Solid", "LightPole", "LavaBall", "LavaBoss", "Watcher"];  //Solid added so that you can hit burnable trees.  
...
{                
    ...
    public function getState() : Void
    {
        ...
        if (tile != null && (new Rectangle(tile.x - tile.originX, tile.y - tile.originY, tile.width, tile.height)).intersects(new Rectangle(x - originX, y - originY + checkOffsetY, width, height)))
        {
    ...
    private function set_slashing(_s : Bool) : Bool
    //slashDirection = -1;
    {
    ...
    public function fire() : Void
    {
        if (firing)
        {
            ...
            if (sprFire.frame >= fireHitFrameStart && sprFire.frame <= fireHitFrameEnd)
            {
                ...
                for (i in 0...hitables.length)
                {
                    FP.world.collideRectInto(Reflect.field(hitables, Std.string(i)), x - sprFire.originX, y - sprFire.originY, sprFire.width, sprFire.height, vc);
                    for (e in vc)
                    {
...
{                            
    ...
    public function genericHit(e : Entity, t : String = "", f : Float, d : Float) : Void
    {
        ...
        {
            (try cast(e, PushableBlockSpear) catch(e:Dynamic) null).hit(new Point(as3hx.Compat.parseInt(spearDirection % 2 == 0) * (spearDirection - 1), as3hx.Compat.parseInt(spearDirection % 2 == 1) * (2 - spearDirection)), t, true);
        }
...
{            
    ...
    public function sprites() : Void
    {
        ...
        if ((v.x == 0 && v.y == 0) || Game.freezeObjects)
        {
            switch (direction)
            {
                case 1:(try cast(graphic, Spritemap) catch(e:Dynamic) null).play(states[state] + "up-stand");
                case 3:(try cast(graphic, Spritemap) catch(e:Dynamic) null).play(states[state] + "down-stand");
                default:(try cast(graphic, Spritemap) catch(e:Dynamic) null).play(states[state] + "side-stand");
            }
        }
    }
