...
load_image_assets();
	...
	override public function update():Void {
		...
		if (vTriggered.length > 0) {
			var player:Player = try cast(FP.world.collideLine("Player", Std.int(x), Std.int(y), Std.int(x + attackRange * (vTriggered.x / vTriggered.length)),
				Std.int(y + attackRange * (vTriggered.y / vTriggered.length))), Player) catch (e:Dynamic) null;
			if (player != null) {
				v = vTriggered;
