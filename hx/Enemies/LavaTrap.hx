...
class LavaTrap extends Enemy {
	...
	private function launch():Void {
		...
		var ind:Int = getTongueLength();
		var pHit:Player = try cast(FP.world.collideLine("Player", Std.int(x), Std.int(y), Std.int(x + tongueLengths[ind] * Math.cos(tongueAngle)),
			Std.int(y + tongueLengths[ind] * Math.sin(tongueAngle))), Player) catch (e:Dynamic) null;
		if (pHit != null) {
			attached = pHit;
		}
	}
