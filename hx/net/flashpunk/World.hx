    ...
    // If the distance is less than precision, do the short sweep.
    {
        ...
        if (xDelta > yDelta)
        {
            ...
            if (xSign > 0)
            {
                while (x < toX)
                {
                    e= collidePoint(type, x, y); if (e != null)
                    {
                        if (p == null)
                        {
                            return e;
...
{                
    ...
    @:allow(net.flashpunk)
    private function addType(e : Entity) : Void
    // add to type list
    {
    ...
    @:allow(net.flashpunk)
    private function removeType(e : Entity) : Void
    ...
    // Adding and removal.
    /** @private */private var _add : Array<Entity> = new Array<Entity>();
    /** @private */private var _remove : Array<Entity> = new Array<Entity>();
    ...
    /** @private */private var _classCount : Dictionary<Class<Dynamic>,Int> = new Dictionary();
    /** @private */@:allow(net.flashpunk)
    private var _typeFirst : Dictionary<String,Entity> = new Dictionary();
    /** @private */private var _typeCount : Dictionary<String,Int> = new Dictionary();
    /** @private */private var _recycled : Dictionary<Class<Dynamic>,Entity> = new Dictionary();
}
