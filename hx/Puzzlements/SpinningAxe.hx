...
class SpinningAxe extends Entity {
	...
	override public function update():Void {
		...
		var p:Player = try cast(FP.world.collideLine("Player", Std.int(x), Std.int(y), Std.int(x + length * Math.cos(a)),
			Std.int(y + length * Math.sin(a))), Player) catch (e:Dynamic) null;
		if (p == null) {
