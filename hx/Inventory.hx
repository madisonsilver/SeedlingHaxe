...
private static var sprTank : Image;
	...
	private var keyPositions(default, never):Array<Dynamic> = [
		new Point(2, 60),
		new Point(15, 60),
		new Point(2, 72),
		new Point(15, 72),
	...
	private static var FUtexts:Array<Dynamic> = [
...
load_image_assets();
	...
	public function update():Void {
		...
		if (p != null && (!Game.freezeObjects || open)) {
			...
			if (open) {
				...
				if (m.x >= pt.x - sprTank.originX
					&& m.x < pt.x - sprTank.originX + sprTank.width
					&& m.y >= pt.y - sprTank.originY
					&& m.y < pt.y - sprTank.originY + sprTank.height) {
	...
	public function render():Void {
		...
		for (i in 0...items.length) {
			...
			sprInventoryItems.render(new Point(Math.ceil(offset.x + itemOffset.x + itemOffsetIncrement.x * i),
				Math.ceil(offset.y + itemOffset.y + itemOffsetIncrement.y * i)),
				new Point());
		}
		...
		t.color = 0x000000;
		t.centerOO();
		t.render(new Point(offset.x
			+ itemOffset.x
			- sprInventoryItems.width / 2
			- t.width / 2,
			offset.y
			+ itemOffset.y
			+ sprInventoryItems.height / 2
			- t.height / 2
			+ itemOffsetIncrement.y * Main.primary),
			new Point());
		t = new Text("C");
		t.scale = textScale[1];
		t.color = 0x000000;
		...
		if (text != null && FP.world.classCount(Help) <= 0 && open) {
			Draw.rect(Std.int(FP.camera.x + helpTextOffset.x), Std.int(FP.camera.y + helpTextOffset.y - text.height / 2 - margin), text.width + margin * 2,
				text.height + margin * 2, 0x000000, 0.95);
	...
	private function ngTankPos():Point {
		var o:Int = 4;
		return new Point(FP.screen.width
			+ o
			- (sprTank.width * 3 / 4 + o) * Math.abs((offset.x - offsetMin.x) / (offsetMax.x - offsetMin.x)),
			FP.screen.height
			- sprTank.height);
	}
