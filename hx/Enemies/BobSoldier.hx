...
class BobSoldier extends Enemy {
	...
	public function swordHitting():Void {
		for (i in 0...swords) {
			var hitPlayer:Player = try cast(FP.world.collideLine("Player", Std.int(x + weaponLength / 2 * Math.cos(-swordSpin[i])),
				Std.int(y + weaponLength / 2 * Math.sin(-swordSpin[i])), Std.int(x + weaponLength * Math.cos(-swordSpin[i])),
				Std.int(y + weaponLength * Math.sin(-swordSpin[i]))), Player) catch (e:Dynamic) null;
			if (hitPlayer != null) {
