...
class Entity extends Tweener
{
    ...
    public function collide(type : String, x : Float, y : Float) : Entity
    {
        ...
        if (_mask == null)
        {
            while (e != null)
            {
                if (x - originX + width > e.x - e.originX && y - originY + height > e.y - e.originY && x - originX < e.x - e.originX + e.width && y - originY < e.y - e.originY + e.height && e.collidable && e != this)
                {
    ...
    public function collideWith(e : Entity, x : Float, y : Float) : Entity
    {
        ...
        if (x - originX + width > e.x - e.originX && y - originY + height > e.y - e.originY && x - originX < e.x - e.originX + e.width && y - originY < e.y - e.originY + e.height && collidable && e.collidable)
        {
            if (_mask == null)
            {
                if (e._mask == null || e._mask.collide(HITBOX))
    ...
    /** @private */private var HITBOX(default, never) : Mask = new Mask();
    /** @private */private var _mask : Mask;
    /** @private */private var _x : Float;
    /** @private */private var _y : Float;
